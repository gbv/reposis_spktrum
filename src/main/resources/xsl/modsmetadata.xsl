...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:xalan="http://xml.apache.org/xalan"
  ...
  <!-- START spktrum specific changes -->
  ...
  <xsl:template name="printRelatedItemAsAPA">
    ...
    <xsl:if test="$mods/mods:location/mods:url[@access='raw object']">
      <xsl:value-of select="' '" />
      <a href="{$mods/mods:location/mods:url[@access='raw object']}" target="_blank">
        <xsl:value-of select="$mods/mods:location/mods:url[@access='raw object']" />
      </a>
    </xsl:if>
  </xsl:template>
  <!-- END spktrum specific changes -->
